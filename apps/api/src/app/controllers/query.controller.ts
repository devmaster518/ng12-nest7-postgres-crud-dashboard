...
import { QueryResult } from '@angular12-nest7-crud-dashboard/api-interfaces';
...
export class QueryController {
  ...
  async getQuery(
    ...
    try {
      ...
      // トークンの正常チェック
      const verifyToken = await this.auth.verifyToken(
      ...
      const sql = fs.readFileSync(
        './apps/api/src/assets/sqls/' + query.sql,
        'utf-8'
      );
      console.log(sql);
  ...
  async postQuery(
    ...
    try {
      ...
      const sql = fs.readFileSync(
        './apps/api/src/assets/sqls/' + body.sql,
        'utf-8'
      );
      console.log(sql);
