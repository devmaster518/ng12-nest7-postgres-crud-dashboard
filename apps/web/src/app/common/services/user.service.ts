...
import {
  ...
  InputType,
} from '../../views/components/simpleDialog/simpleDialog.component';
...
export class UserService {
  ...
  async login(
    ...
    try {
      ...
      // トークン生成APIの呼び出し
      const ret: any = await this.http.post('api/createToken', {
        userCode: userCode,
  ...
  async getUser(userId: string | null) {
    try {
      ...
      const ret: any = await this.http.get(
        'api/query?sql=Users/getUser.sql&values=' + values
      );
      if (ret.message !== null) {
