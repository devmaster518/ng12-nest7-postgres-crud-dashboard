...
export class SimpleDialogComponent {
  ...
  public open(width?: string): SimpleDialogComponent {
    ...
    const dialogComponent =
      dialogRef.componentInstance as SimpleDialogComponent;
      ...
      for (let i = 0; i < this.items.length; i++) {
        ...
        if (this.items[i].required === true) {
          if (
            ...
            this.items[i].validateMessage =
              this.translate.instant('requiredError');
            ret = false;
          }
        }
      }
  ...
  selectFilter(item: any): void {
    ...
    item.filteredSelectList = item.selectList.filter((s: any) =>
      s.name.toLowerCase().includes(item.selectListFilter || '')
    );
  }
  ...
  selectChanged(item: any, data: any) {
    ...
    item.selectListFilter = item.selectList.find(
  ...
  async onSelectFiles(event: any, item: any) {
    ...
    if (item.maxsize) {
      ...
      if (sizeMB > item.maxsize) {
        alert(
          `${item.maxsize}MB以下のファイルを選択してください。ファイルサイズ：${sizeMB}MB`
        );
        return;
      }
    }
