...
export class SearchConditionComponent implements OnInit {
  ...
  async openSearchDialog() {
    ...
    for (let i = 0; i < dialog.items.length; i++) {
      // キャンセルして開きなおしたときのために、valuesをitemsに戻す
    ...
    localStorage.setItem(
      'searchCondition.' + this.searchConditionName,
      JSON.stringify(this.values)
    );
    console.log(
      `save search condition. [${this.searchConditionName}]` +
        JSON.stringify(this.values)
    );
  }
    ...
    for (let i = 0; i < this.values.length; i++) {
      if (!Array.isArray(this.values[i])) {
        if (this.values[i]) {
          if (str) str += ', ';
          str +=
            this.translate.instant(this.items[i].label) + '：' + this.values[i];
        }
      } else {
        if (this.values[i][0] || this.values[i][1]) {
          if (str) str += ', ';
          str +=
            this.translate.instant(this.items[i].label) +
            '：' +
            (this.values[i][0] || '') +
            '～' +
            (this.values[i][1] || '');
  ...
  createDefaultValues() {
    ...
    for (let i = 0; i < this.items.length; i++) {
      if (this.items[i].inputtype.substr(-5) == 'Range') {
        // 範囲入力の場合は配列をセット
  ...
  // ローカルストレージの検索条件を復元
  restoreSetting() {
    var searchConditionStr = localStorage.getItem(
      'searchCondition.' + this.searchConditionName
    );
    if (searchConditionStr) {
      this.values = JSON.parse(searchConditionStr);
      console.log(
        `restore search condition. [${this.searchConditionName}] ` +
          searchConditionStr
      );
    } else {
