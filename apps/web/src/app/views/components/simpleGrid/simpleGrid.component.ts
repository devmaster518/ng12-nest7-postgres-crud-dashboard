...
export class SimpleGridComponent implements OnChanges {
    ...
    if (this.columnDefine && this.dataSource) {
      ...
      setTimeout(function () {
        ...
        if (colWidthStr) {
          ...
          console.log(
            `restore table column width. [${self.gridName}] ` + colWidthStr
          );
        }
  ...
  onResized(event: ResizedEvent) {
    ...
    setTimeout(function () {
      ...
      if (
        SimpleGridComponent.lastEvent &&
        lastEvent.time < SimpleGridComponent.lastEvent.time
      )
        return;
      ...
      if (saveFlag) {
        localStorage.setItem(
          'tableCoWidth.' + self.gridName,
          JSON.stringify(colWidth)
        );
        console.log(
          `save table column width. [${self.gridName}] ` + colWidth.join(',')
        );
      }
