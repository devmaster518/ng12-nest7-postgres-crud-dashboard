...
export class UserMngComponent implements OnInit, AfterViewInit {
  ...
  columnDefine: ColumnDefine[] = [
    ...
    { type: 'string', column: 'note', name: 'note' },
    {
      ...
      name: 'edit',
      icon: 'edit',
      method: async (data: any) => await this.regUser(data),
      color: 'primary',
      auth: 22,
    },
    {
      ...
      name: 'delete',
      icon: 'delete_forever',
      method: async (data: any) => await this.delUser(data),
      color: 'warn',
      auth: 23,
    },
  ];
  ...
  async regUserExec(data: any, dialog: SimpleDialogComponent) {
    ...
    try {
      ...
      const auth = JSON.stringify(
        Enums.getSelectedValue(Enums.Auth, dialog.items[8].value)
      );
      ...
      if (data) {
        body = {
          ...
          values: [code, name, age, sex, birthday, note, auth, data.id],
        };
      } else {
        // パスワードのハッシュ化
