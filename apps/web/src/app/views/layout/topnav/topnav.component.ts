...
import {
  ...
  InputType,
} from '../../components/simpleDialog/simpleDialog.component';
...
@Component({
  ...
  providers: [HttpRequestInterceptor, DatePipe, EnumChangePipe],
})
export class TopnavComponent implements OnInit {
  ...
  // ユーザ情報表示
  async showUserInfo() {
    const selectedName = this.user.auth.map((id) =>
      this.enumChangePipe.transform(id, Enums.Auth)
    );
    ...
    dialog.items = [
      ...
      {
        label: 'user.sex',
        value: this.enumChangePipe.transform(this.user.sex, Enums.Sex),
        inputtype: InputType.Display,
      },
      {
        label: 'user.birthday',
        value: this.datePipe.transform(this.user.birthday, 'yyyy/MM/dd'),
        inputtype: InputType.Display,
      },
      {
        label: 'user.note',
        value: this.user.note,
        inputtype: InputType.DisplayArea,
      },
      {
        label: 'user.auth',
        value: selectedName,
  ...
  async openUserMng() {
    ...
    dialog.updateData = async (data: any) =>
      await this.updateUser(dialog, data);
    dialog.deleteData = async (data: any) =>
      await this.deleteUser(dialog, data);
    dialog.list_columns = [
      'code',
      'name',
      'age',
  ...
  async getUsers(dialog: TableDialogComponent) {
    ...
    const ret: any = await this.http.get(
      'api/query?sql=Users/getUsers.sql&values=' + values
    );
    if (ret.message !== null) {
    ...
    datas.forEach((data: any) => {
      ...
      data['birthdayDisp'] = this.datePipe.transform(
        data['birthday'],
        'yyyy/MM/dd'
      );
    });
  ...
  async regUser(dialog: TableDialogComponent, data: any) {
    ...
    dialog2.items = [
      {
        label: 'user.code',
        value: data?.code,
        inputtype: data ? InputType.Display : InputType.Text,
        required: true,
        placeholder: '',
      },
      {
      ...
      },
      {
        label: 'user.password',
        value: data ? '●●●●●●' : null,
        inputtype: data ? InputType.Display : InputType.Password,
        required: true,
        placeholder: '',
      },
      {
    ...
    dialog2.buttons = [
      ...
      },
      {
        ...
        name: 'ok',
        click: async () => {
          await this.regUserExec(dialog2, data);
        },
      },
    ];
  ...
  async regUserExec(dialog: SimpleDialogComponent, data: any) {
    ...
    const auth = JSON.stringify(
      Enums.getSelectedValue(Enums.Auth, dialog.items[7].value)
    );
    ...
    if (data) {
      body = {
        sql: 'Users/updUser.sql',
        values: [data._id, code, name, age, sex, birthday, note, auth],
      };
    } else {
      // パスワードのハッシュ化
      const hashedPassword = crypto.SHA512(password).toString();
      body = {
        sql: 'Users/addUser.sql',
        values: [code, name, age, sex, birthday, hashedPassword, note, auth],
      };
    }
  ...
  async changePassword() {
    ...
    dialog.buttons = [
      ...
      },
      {
        ...
        name: 'ok',
        click: async () => {
          await this.changePasswordExec(dialog);
        },
      },
    ];
  ...
  async openSystemInfo() {
    ...
    dialog.items = [
      {
        label: 'system.clientBuildDt',
        value: systemInfo.clientBuildDt,
        inputtype: InputType.Display,
      },
      {
        label: 'system.serverBuildDt',
        value: systemInfo.serverBuildDt,
        inputtype: InputType.Display,
      },
      {
        label: 'system.gitLog',
        value: systemInfo.gitLog,
        inputtype: InputType.DisplayArea,
        rows: 20,
        cols: 100,
      },
